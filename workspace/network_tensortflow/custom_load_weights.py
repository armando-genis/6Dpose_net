...
def load_weights_from_hdf5_group_by_name(
  ...
  if 'keras_version' in f.attrs:
    # Handle both bytes and string types for keras_version
  ...
  if 'backend' in f.attrs:
    # Handle both bytes and string types for backend
...
def load_attributes_from_hdf5_group(group, name):
  ...
  if name in group.attrs:
    attr_data = group.attrs[name]
    # Check if attr_data is empty before trying to access its first element
    if len(attr_data) > 0:
      if isinstance(attr_data[0], bytes):
          data = [n.decode('utf8') for n in attr_data]
      else:
          data = [n for n in attr_data]
    else:
      # Handle empty attribute data
      data = []
  else:
    data = []
    chunk_id = 0
    while '%s%d' % (name, chunk_id) in group.attrs:
      attr_data = group.attrs['%s%d' % (name, chunk_id)]
      # Also check for empty chunks
      if len(attr_data) > 0:
        if isinstance(attr_data[0], bytes):
            data.extend([n.decode('utf8') for n in attr_data])
        else:
            data.extend([n for n in attr_data])
      chunk_id += 1
  return data
